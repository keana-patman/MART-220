...
function preload()
{
    ...
    for (var i = 0; i < 15; i++) 
    {
        zombie = new imageclass("character/Idle (" +i+ ").png",x ,y);
        idleAnimation.push(zombie);
    }

    for (var j = 0; j < 10; j++)
    {
        zombie = new imageclass("character/Walk (" + j + ").png",x ,y);
...
function setup()
{
    ...
    for(var k = 0; k < 7; k++)
    {
        myBean[k] = new foodclass(random(450),random(450));
    }
}
...
function draw()
{
    ...
    //random green beans
    for(var k = 0; k < myBean.length; k++)
    {
        myBean[k].createBean();
    }

    for (let k = 0; k < myBean.length; k++) 
    {
        if (walkAnimation[j].hasCollided(myBean[k].gX, myBean[k].gY, 10, 5)) 
        {
            myBean.splice(k, 1); 
        }
    }
